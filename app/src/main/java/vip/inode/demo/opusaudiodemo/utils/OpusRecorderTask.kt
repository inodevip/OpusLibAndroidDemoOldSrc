/*
 * Coding by Zhonghua. from 18-9-30 上午11:04.
 */
...
import vip.inode.demo.opusaudiodemo.presenter.DecodeOpusPresenter.Companion.BUFFER_LENGTH
import vip.inode.demo.opusaudiodemo.presenter.DecodeOpusPresenter.Companion.DEFAULT_AUDIO_SAMPLE_RATE
import vip.inode.demo.opusaudiodemo.presenter.DecodeOpusPresenter.Companion.DEFAULT_OPUS_CHANNEL
import java.io.BufferedOutputStream
import java.io.File
import java.io.FileOutputStream

class OpusRecorderTask(
  private val opusAudioOpusPath: String,
  private val opusAudioPcmPath: String
) : Runnable {
  companion object {
    private var channelConfig = AudioFormat.CHANNEL_IN_MONO
    val bufferSize: Int = AudioRecord.getMinBufferSize(
        DEFAULT_AUDIO_SAMPLE_RATE, channelConfig, AudioFormat.ENCODING_PCM_16BIT
    )
  }
  ...
  init {
    audioRecord = AudioRecord(
        MediaRecorder.AudioSource.MIC, DEFAULT_AUDIO_SAMPLE_RATE, channelConfig,
  ...
  override fun run() {
    ...
    while (isRecorder) {
      ...
      if (curShortSize > 0 && curShortSize <= audioBuffer.size) {
        ...
        val encodeSize =
          opusUtils.encode(createEncoder, Uilts.byteArrayToShortArray(audioBuffer), 0, byteArray)
        if (encodeSize > 0) {
